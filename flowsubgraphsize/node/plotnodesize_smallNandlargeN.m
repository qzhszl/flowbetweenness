...
for N = [10,20,30,50]
    ...
    plot(result_table.RealAveDegree,result_table.SizeFSG/N,'Marker', 'p','LineStyle','none', 'MarkerSize', 6,'MarkerEdgeColor',colors(count), 'MarkerFaceColor', colors(count))
    hold on
    count = count+1;
end
...
ylabel('$E\left[\rho_N\right]$','interpreter','latex',FontSize=16)
ylim = ([0,1.05]);
xlim = ([0,10]);
...
ax = gca;  % Get current axis
ax.FontSize = 12;  % Set font size for tick label
picname = sprintf("D:\\data\\flow betweenness\\sizeofflowsubgraph\\new\\size_flow_subgraph_large.pdf");
exportgraphics(fig, picname,'BackgroundColor', 'none','Resolution', 600);