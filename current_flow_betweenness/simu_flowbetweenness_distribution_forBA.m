function simu_flowbetweenness_distribution_forBA()
    ...
    filefolder_name_input = "D:\\data\\flow betweenness\\BA_network\\";
    
    input_BAname = 'BAnetworkN5000m3.txt';
    ...
    for r = 1:R
        ...
        elseif strcmp(model,'BA')
            data = readmatrix(filefolder_name_input+input_BAname);  % 每行: u v w
            % 提取三列
            s = data(:,1)+1;    % 起点
            t = data(:,2)+1;    % 终点
            w = data(:,3);    % 权重
    ...
    pos  =pos(end);
    filename = [filename(1:pos), insertStrNoTxt, filename(pos+1:end)];
    % 保存到文件
