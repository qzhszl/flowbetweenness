...
for N = N_vec
    p_vec = linspace(p_start_vec(countN), 0.5, 12);
    p_vec = round(p_vec,4);
    countp = 1;
    for p= p_vec
...
for countplot = 1:length(N_vec)
    p_vec = linspace(p_start_vec(countplot), 0.5, 12);
    p_vec = round(p_vec,4);
    errorbar(p_vec, data_mean(:,countplot), data_std(:,countplot), 'o-', 'Color', colors(countplot), 'LineWidth', 4, 'MarkerSize', 8,'CapSize',8);
end
...
ylabel('$\frac{N}{N(N-1)}\sum_i\sum_j\frac{|d_{ij}-c_{ij}|}{d_{ij}}$','interpreter','latex',FontSize=30)
lgd = legend({'$N = 10$', '$N = 20$', '$N = 50$', '$N = 100$'}, 'interpreter','latex','Location', 'northwest',FontSize=30);
% lgd.NumColumns = 2;
% set(legend, 'Position', [0.446, 0.73, 0.2, 0.1]);
box on
hold off

picname = sprintf("D:\\data\\flow betweenness\\IERP\\IERP_ER.pdf");
