function Simulations_on_emperical_network_server()
    ...
    filename = "Newspain_clean.txt";

    filename = folder_name+filename;
    if ~isfile(filename)
        error('找不到输入文件：%s', filename);
    end
    ...
    A_input = full(G_all.adjacency("weighted"));
    NODENUM = size(A_input,1)
    Linknum = nnz(A_input)/2
    ...
    % filename = sprintf("IERPspain.txt");
