...
h_bar  = errorbar(p_vec, -5*ones(length(p_vec),1), zeros(length(p_vec),1), 'o-', 'Color', 'black', 'LineWidth', 1, 'MarkerSize', 14,'CapSize',12);     
...
for countplot = 1:length(N_vec)
    ...
    errorbar(p_vec, ones(length(p_vec),1), zeros(length(p_vec),1), 'p', 'Color', colors(countplot), 'LineWidth', 4, 'MarkerSize', 10,'CapSize',8);
end
...
ylabel('$L_c/L_H$','interpreter','latex',FontSize=36)
lgd = legend({'Fiedler','RGP','$N = 20$', '$N = 50$', '$N = 100$', '$N = 200$'}, 'interpreter','latex','Location', 'southeast',FontSize=30);
lgd.NumColumns = 2;
lgd.ItemTokenSize = [25, 40];
box on
hold off

picname = sprintf("D:\\data\\flow betweenness\\IERP\\IERP_ER_commonlink_compare2alg.pdf");
exportgraphics(fig, picname,'BackgroundColor', 'none','Resolution', 600);

picname = sprintf("D:\\data\\flow betweenness\\IERP\\IERP_ER_commonlink_compare2alg.svg");
